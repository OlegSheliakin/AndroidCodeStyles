...
Названия любых Android ресурсов должны соответствовать `lowercase_underscope`, а также соответствующему паттерну.
...
ID элементов верстки должны соответствовать `lowercase_underscope` и паттерну - `%type%_%description%`.
...
Названия строковых ресурсов должны соответствовать `lowercase_underscope` и паттерну - `%feature%_%type%_%description%`.
...
Названия цветов должны соответствовать `lowercase_underscope` и паттерну `%feature%_%component%_%color_name%_%description%_%opacity%`.
...
Названия размеров должны соответствовать `lowercase_underscope` и паттерну `%feature%_%component%_%dimen_type%_%description%`.
...
Если `if` или `when` возврашают значени, то return можно вынести
```kotlin
    return if (foo) {
        bar()
    } else { 
...
Если лямба выражение пишется в одной строке и имеет один параметр, то следует использовать `it` в качестве параметра
```kotlin
    foo { it > 10 }
```
...
При использовании лямба-выражения в качестве аругмента следует выносить её за скобки, если этот параметр последний
```kotlin
    foo(bar, baz) { it > 10 }
...
```kotlin
    ...
    class ProfileRepositroyImpl : ProfileRepositroy {
        ...
    }
```
